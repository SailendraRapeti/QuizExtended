...
class Quiz1 extends Component<Iprops, Istate> {
  state = {
    checked: '',
    err: '',
  };
  onNext = () => {
    ...
    } else if (checked === 'Both A and B') {
      this.props.navigation.navigate('Quiz2', {totalScore: 1});
    } else {
      this.props.navigation.navigate('Quiz2', {totalScore: 0});
    }
  };
  render() {
    return (
      <View style={style.mainContainer}>
        ...
        <View style={{padding: 20}}>
          ...
          <View style={style.radio}>
            <RadioButton
              value="var"
              status={this.state.checked === 'var' ? 'checked' : 'unchecked'} //if the value of checked is Apple, then select this button
              onPress={() => this.setState({checked: 'var'})} //when pressed, set the value of the checked Hook to 'Apple'
            />
            <Text
              style={{
                padding: 5,
